...
def _correlations(y_true, y_pred):
    """Spearman ρ 与 Pearson r；优先用 scipy，缺失时回退 numpy 实现（部署精简包无 scipy 也能算）。"""
    try:
        ...
        rt = np.asarray(y_true, dtype=float).argsort().argsort().astype(float)
        rp = np.asarray(y_pred, dtype=float).argsort().argsort().astype(float)
...
def _model_predict(model, X, num_iteration=None):
    if hasattr(model, "feature_importance"):   # lightgbm.Booster
        if num_iteration:
            return np.asarray(model.predict(np.asarray(X, dtype=float), num_iteration=num_iteration), dtype=float)
...
def train_model():
    ...
    # 训练依赖 sklearn / scipy（仅训练时使用；部署后的预测接口不需要它们）
    ...
    try:
        ...
        bundle = {
            "model": final_model.booster_,
            "scaler": {"min_": np.asarray(scaler.min_, dtype=float),
                       "scale_": np.asarray(scaler.scale_, dtype=float)},
            "best_iteration": int(final_model.best_iteration_) if getattr(final_model, "best_iteration_", None) else None,
...
def load_bundle():
    ...
    if os.path.exists(BUNDLE_PATH):
        try:
            ...
            log.error("加载模型 bundle 失败（可能需要用 mordred 1.2.0 重新训练）：%s", e)
            bundle = None
            return False
    return False
...
def predict():
    ...
    try:
        ...
        X_scaled = _apply_scaler(scaler, X_sel)
        pred = _model_predict(model, X_scaled, best_iter)
...
def structure_visualization():
    ...
    try:
        ...
        X_sel = X_full.iloc[:, selected_indices].copy()
        X_scaled = _apply_scaler(scaler, X_sel)
        pred = _model_predict(model, X_scaled, best_iter)[0]
